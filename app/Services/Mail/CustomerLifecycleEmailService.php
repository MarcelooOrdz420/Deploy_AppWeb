...
HTML;
    ...
    public function sendPromotion(User $user, array $campaign): void
    {
        ...
        $ctaPath = trim((string) ($campaign['cta_url'] ?? '/productos'));
        $ctaUrl = str_starts_with($ctaPath, 'http://') || str_starts_with($ctaPath, 'https://') ? $ctaPath : url('/'.ltrim($ctaPath, '/'));
        $safeCtaUrl = e($ctaUrl);
        ...
        $imageBlock = $imageSrc !== ''
            ? '<div style="padding:20px;display:flex;align-items:center;justify-content:center;background:radial-gradient(circle at center, rgba(255,255,255,.18), transparent 44%), linear-gradient(135deg,#ff7c18 0%,#ff931f 100%);"><img src="'.$imageSrc.'" alt="'.$title.'" style="width:100%;max-width:280px;height:280px;object-fit:cover;border-radius:28px;display:block;border:4px solid rgba(255,255,255,.34);box-shadow:0 22px 38px rgba(84,32,0,.22);"></div>'

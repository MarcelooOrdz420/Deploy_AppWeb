    ...
    <section class="checkout-shell">
        ...
        <div class="checkout-board">
            <aside class="checkout-sidebar">
                ...
                <section id="pendingOrderBox" class="surface recent-order-panel" style="display:none;"></section>
...
async function checkPendingIzipayOrder(){
    ...
    try{
        ...
        // Fallo de red pasajero: se vuelve a revisar la proxima vez que cargue el carrito.
    }
}

function showPendingOrderBox(pending){
    pendingOrderBox.style.display='block';
    pendingOrderBox.innerHTML=`<strong>Tienes un pago pendiente: ${pending.tracking}</strong><p>Empezaste un pago con tarjeta que no se completo. Puedes continuarlo o cancelarlo; si lo cancelas, tu carrito se vaciara.</p><div style="display:flex;gap:10px;margin-top:6px;"><button type="button" class="btn-main" id="pendingOrderContinueBtn" style="flex:1;">Continuar pago</button><button type="button" class="pill-btn" id="pendingOrderCancelBtn" style="flex:1;">Cancelar pedido</button></div>`;
...
orderForm.addEventListener('submit', async e => {
    ...
    try {
        ...
        if (storedPaymentMethod === 'izipay') {
            ...
            // quien realmente vacia 'ed_cart' segun corresponda. Guardamos la
            // referencia del pedido para poder ofrecer continuar/cancelar si
            // el cliente vuelve al carrito sin pasar por esa pagina (ej. le
            // dio "atras" en el navegador antes de terminar el pago).

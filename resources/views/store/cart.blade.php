                ...
                <form id="orderForm" class="checkout-form">
                    ...
                    <section class="surface form-stage payment-stage">
                        ...
                        <label id="payOptions" class="field-card payment-method-field">
                            ...
                            <span class="field-help">Elige si pagaras ahora con tarjeta o al recibir tu pedido.</span>
                        </label>
...
@section('scripts')
<script>
const COMPANY={brand_name:'Pollos y Parrillas El Dorado',payments:{izipay:{label:'Pago con tarjeta',message:'Paga con tarjeta desde el checkout seguro de Izipay.',public_key:''},cod:{label:'Pago contraentrega',message:'Paga al recibir tu pedido en el lugar de entrega acordado.',enabled:true}}};
...
async function loadCompanySettings(){try{const res=await fetch('/api/v1/settings/public'),data=await res.json();if(res.ok&&data&&data.payments){COMPANY.brand_name=data.brand_name||COMPANY.brand_name;COMPANY.payments=data.payments}}catch{}}
 function updatePaymentInfo(){const method=paymentMethod(),settings=COMPANY.payments?.[method]||{},isIzipay=method==='izipay';if(paymentProofWrap)paymentProofWrap.style.display='none';updateHeroStatus(getCart());paymentInfo.innerHTML=method?`<span class="hero-kpi-label">Pago elegido</span><strong>${settings.label||(isIzipay?'Pago con tarjeta':'Pago contraentrega')}</strong><div>${settings.message||(isIzipay?'Paga con tarjeta desde el checkout seguro de Izipay.':'Paga al recibir tu pedido en el lugar de entrega acordado.')}</div>${isIzipay?'<div>No compartes los datos de tu tarjeta con esta web.</div>':'<div>Ten listo el monto al momento de la entrega.</div>'}`:'<span class="hero-kpi-label">Pago</span><strong>Sin metodo seleccionado</strong>'}

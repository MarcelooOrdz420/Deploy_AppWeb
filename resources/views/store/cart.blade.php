                    ...
                    <section class="surface form-stage payment-stage">
                        ...
                        <div id="payOptions" class="pay-stage-grid">
                            ...
                            <label class="pay-tile" data-method="yape">
                                ...
                                <span class="pay-icon" aria-hidden="true">
                                    <svg width="22" height="22" viewBox="0 0 24 24" fill="none">
                                        ...
                                        <path d="M16 15.5l1.5 1.5L21 13.5" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/>
                                    </svg>
                                </span>
                                <div class="pay-tile-body">
                                    <span class="pay-tile-kicker">Billetera</span>
                                    <strong>Yape</strong>
                                    <small>Desde el checkout Izipay.</small>
                                </div>
                            </label>
                            <label class="pay-tile" data-method="plin">
                                ...
                                <span class="pay-icon" aria-hidden="true">
                                    <svg width="22" height="22" viewBox="0 0 24 24" fill="none">
                                        <path d="M12 3v18M6 8.5h8.5a3.5 3.5 0 0 1 0 7H6" stroke="currentColor" stroke-width="1.8" stroke-linecap="round"/>
                                        <path d="M17 5l3 3-3 3" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/>
                                    </svg>
                                </span>
                                <div class="pay-tile-body">
                                    <span class="pay-tile-kicker">Billetera</span>
...
@section('scripts')
<script>
const COMPANY={brand_name:'Pollos y Parrillas El Dorado',payments:{izipay:{label:'Pago seguro con Izipay',message:'Paga con tarjeta desde el checkout seguro de Izipay.',public_key:''},yape:{label:'Yape con Izipay',message:'Elige Yape en el checkout seguro de Izipay y confirma el pago desde tu app.',public_key:''},plin:{label:'Plin con Izipay',message:'Elige Plin en el checkout seguro de Izipay y completa el pago en tu billetera.',public_key:''}}};
...
function usesIzipayCheckout(method){return ['izipay','yape','plin'].includes(method)}
...
function updatePaymentInfo(){const method=paymentMethod(),payment=COMPANY.payments?.[method]||COMPANY.payments?.izipay||{};payOptions.querySelectorAll('.pay-tile').forEach(option=>option.classList.toggle('is-active',option.getAttribute('data-method')===method));if(paymentProofWrap)paymentProofWrap.style.display='none';updateHeroStatus(getCart());paymentInfo.innerHTML=`<span class="hero-kpi-label">Pago elegido</span><strong>${payment.label||'Pago seguro con Izipay'}</strong><div>${payment.message||'Completa tu pago desde el checkout seguro de Izipay.'}</div><div>Izipay procesara este pago y notificara automaticamente el resultado.</div>`}
...
orderForm.addEventListener('submit', async e => {
    ...
    try {
        ...
        if (usesIzipayCheckout(paymentMethod())) {

...
function applyPolliaCheckoutPrefill(){try{const raw=localStorage.getItem('ed_pollia_checkout_prefill_v1');if(!raw)return;const data=JSON.parse(raw);if(data.phone&&!orderForm.customer_phone.value.trim())orderForm.customer_phone.value=data.phone;if(data.email&&!billingEmail.value.trim())billingEmail.value=data.email;if(data.address&&!orderForm.address.value.trim()){orderForm.delivery_type.value='delivery';orderForm.address.value=data.address}if(data.reference&&!orderForm.reference.value.trim())orderForm.reference.value=data.reference;localStorage.removeItem('ed_pollia_checkout_prefill_v1')}catch{}}

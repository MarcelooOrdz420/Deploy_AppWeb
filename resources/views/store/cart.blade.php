...
function updatePaymentInfo(){const method=paymentMethod(),izipay=COMPANY.payments?.izipay||{};payOptions.querySelectorAll('.pay-tile').forEach(option=>option.classList.toggle('is-active',option.getAttribute('data-method')===method));if(paymentProofWrap)paymentProofWrap.style.display='none';updateHeroStatus(getCart());paymentInfo.innerHTML=`<span class="hero-kpi-label">Pago elegido</span><strong>${izipay.label||'Pago seguro con Izipay'}</strong><div>${izipay.message||'Paga con tarjeta desde el checkout seguro de Izipay.'}</div><div>No compartes los datos de tu tarjeta con esta web.</div>`}

...
@section('content')
        ...
        <div class="checkout-board">
            <aside class="checkout-sidebar">
                <section class="surface summary-panel">
                    ...
                    <div id="deliveryFeeRow" class="info-ribbon" style="display:none;"></div>
            ...
            <section class="checkout-flow">
                <form id="orderForm" class="checkout-form">
                    ...
                    <section class="surface form-stage">
                        ...
                        <label class="field-card" style="margin-bottom:14px; display:flex; flex-direction:row; align-items:center; gap:10px; cursor:pointer;">
                            <input type="checkbox" id="otherPersonToggle" style="width:18px; height:18px; flex-shrink:0;">
                            <span class="label-main" style="margin:0;">Otra persona recogera o recibira este pedido</span>
                        </label>
                        <div id="customerNameWrap" class="form-grid" style="display:none;margin-bottom:14px;">
                            <label class="field-card">
                                <span class="label-main" id="customerNameLabel">Nombre de quien recogera o recibira</span>
                                <input class="input-main" name="customer_name" placeholder="Ej: Juan Perez">
                            </label>
                        </div>
                        ...
                        <div class="form-grid">
                            <label class="field-card">
                                ...
                                <small class="muted-main" style="display:block;margin-top:6px;font-size:12px;">Ingresa un numero real y activo: el repartidor lo usara para coordinar contigo la entrega.</small>
                            </label>
                            <label class="field-card">
                                <span class="label-main">Indicaciones para tu pedido (opcional)</span>
                                <textarea class="input-main" name="drink_note" maxlength="120" rows="2" placeholder="Ej: mas cremas, sin aji, poco picante..."></textarea>
                            </label>
                        </div>
                    </section>
                    ...
                    <section class="surface form-stage payment-stage">
                        ...
                        <label id="payOptions" class="field-card payment-method-field">
                            ...
                            <span class="field-help">Paga con tarjeta mediante Izipay o al recibir tu pedido.</span>
                        </label>
                    ...
                    <section class="surface form-stage">
                        ...
                        <div id="billingFieldsWrap" class="form-grid extended-grid" style="display:none;">
                            ...
                            <div class="field-card">
                                ...
                                <p id="receiptDeliveryFixedNote" class="muted-main" style="margin:6px 0 0;">Se enviara automaticamente a tu correo asociado.</p>
                                <div id="receiptDeliveryChoice" class="choice-cluster" style="display:none;">
                                    <label class="choice-chip"><input type="radio" name="receipt_delivery" value="registered_email" checked> A mi correo registrado</label>
                    ...
                    <section class="surface form-stage">
                        ...
                        <div id="deliveryFieldsWrap" class="form-grid extended-grid" style="display:none;">
                            ...
                            <label class="field-card">
                                <span class="label-main">Numero de casa / cuadra (opcional)</span>
                                <input class="input-main" name="house_number" placeholder="Ej: casa 245, cuadra 3">
                            </label>
                        </div>
    ...
    <style>
        ...
        .choice-chip:has(input:checked){background:#ff6f1f;border-color:#ff6f1f;color:#fff}
...
function selectedReceiptDelivery(){const checked=orderForm.querySelector('input[name="receipt_delivery"]:checked');return checked?checked.value:'registered_email'}
function updateReceiptDeliveryUi(){const currentUser=typeof window.parseUser==='function'?window.parseUser():null;if(!currentReceiptType()){receiptDeliveryFixedNote.style.display='none';receiptDeliveryChoice.style.display='none';billingEmailWrap.style.display='none';billingEmail.value='';return}const isFactura=currentReceiptType()==='factura';if(!isFactura){receiptDeliveryFixedNote.style.display='block';receiptDeliveryChoice.style.display='none';billingEmailWrap.style.display='none';billingEmail.readOnly=true;billingEmail.value=currentUser?.email||'';return}receiptDeliveryFixedNote.style.display='none';receiptDeliveryChoice.style.display='flex';const mode=selectedReceiptDelivery();billingEmailWrap.style.display='grid';if(mode==='registered_email'){billingEmail.value=currentUser?.email||'';billingEmail.readOnly=true}else{billingEmail.readOnly=false;if(billingEmail.value===currentUser?.email)billingEmail.value=''}}
function updateDeliveryUi(){const isDelivery=deliveryType.value==='delivery';deliveryFieldsWrap.style.display=isDelivery?'grid':'none';geoBtn.style.display=isDelivery?'inline-flex':'none';geoMsg.style.display='none';if(!isDelivery){orderForm.address.value='';orderForm.reference.value='';if(orderForm.house_number)orderForm.house_number.value='';if(orderForm.latitude)orderForm.latitude.value='';if(orderForm.longitude)orderForm.longitude.value=''}else{showManualLocationWarningIfNeeded()}}
function showManualLocationWarningIfNeeded(){if(orderForm.latitude&&orderForm.latitude.value)return;if(!orderForm.address.value.trim())return;geoMsg.style.display='block';geoMsg.textContent='Estas escribiendo la ubicacion manualmente. Verifica que este bien escrita o usa "Usar mi ubicacion exacta" para que el repartidor te ubique con precision.'}
function combinedReference(){const reference=orderForm.reference.value.trim(),houseNumber=orderForm.house_number?orderForm.house_number.value.trim():'';if(!houseNumber)return reference;return[reference,`Nro./Cuadra: ${houseNumber}`].filter(Boolean).join(' | ')}
function handleDeliveryTypeChange(){if(deliveryType.value==='delivery'){const subtotal=getCart().reduce((sum,item)=>sum+Number(item.price)*Number(item.qty),0);if(!qualifiesForDelivery(subtotal)){deliveryType.value='pickup';updateDeliveryUi();renderCart();alertDeliveryBlocked();return}alertDeliveryFee()}updateDeliveryUi();renderCart()}
async function reverseGeocode(latitude,longitude){const controller=new AbortController(),timeout=setTimeout(()=>controller.abort(),8000),url=`https://nominatim.openstreetmap.org/reverse?format=jsonv2&lat=${encodeURIComponent(latitude)}&lon=${encodeURIComponent(longitude)}&zoom=18&addressdetails=1`;try{const res=await fetch(url,{headers:{'Accept':'application/json'},signal:controller.signal});if(!res.ok)throw new Error('No se pudo traducir la ubicacion a calles cercanas.');return res.json()}finally{clearTimeout(timeout)}}
 function updatePaymentInfo(){if(paymentProofWrap)paymentProofWrap.style.display='none';updateHeroStatus(getCart())}
 function resetCheckoutAfterLogout(){orderForm.reset();resetBillingFields();billingReceiptType.value='';deliveryType.value='pickup';if(paymentProofFile)paymentProofFile.value='';if(paymentProofPreview){paymentProofPreview.removeAttribute('src');paymentProofPreview.style.display='none'}lastOrderBox.style.display='none';lastOrderBox.innerHTML='';orderMsg.textContent='';renderCart();updateBillingUi();updateDeliveryUi()}
...
async function showLastOrder(){
    const raw=localStorage.getItem('ed_last_tracking');
    if(!raw)return;
    let info;
...
function updateOtherPersonUi(){const active=otherPersonToggle.checked;customerNameWrap.style.display=active?'grid':'none';orderForm.customer_name.required=active;if(!active)orderForm.customer_name.value=''}
otherPersonToggle.addEventListener('change',updateOtherPersonUi);
updateOtherPersonUi();
orderForm.addEventListener('submit', async e => {
    ...
    if (cartSubtotal < MINIMUM_ORDER_SUBTOTAL) {
        showStoreAlert('Pedido muy pequeño', 'El pedido debe ser de al menos S/10.00. Agrega mas productos para continuar.');
        return;
    }

    if (deliveryType.value === 'delivery') {
        const subtotal = cart.reduce((sum, item) => sum + Number(item.price) * Number(item.qty), 0);
        if (!qualifiesForDelivery(subtotal)) {
            orderMsg.textContent = 'Los pedidos menores a S/10 no califican para delivery. Elige recojo en tienda o agrega mas productos.';
            return;
        }
    }
    ...
    const payload = {
        ...
        customer_name: orderForm.customer_name.value.trim() || null,
        ...
        reference: combinedReference() || null,
    ...
    try {
        ...
        localStorage.setItem('ed_last_tracking', JSON.stringify({ id: data.id, tracking: data.tracking_code }));
        ...
        if (storedPaymentMethod === 'izipay') {
            // El pedido ya quedo registrado y de aqui en adelante se maneja
            // desde Mis pedidos (continuar pago, cancelar, etc.), no desde el
            // carrito. Por eso se vacia el carrito ya mismo: no tiene sentido
            // dejarlo con productos "fantasma" de un pedido que ya existe.
            localStorage.setItem('ed_pending_izipay_order', JSON.stringify({ id: data.id, tracking: data.tracking_code }));
            setCart([]);
            renderCart();
